...
def single_cc_check(message):
    if str(message.chat.id) not in allowed_users:
        bot.reply_to(message, "🚫 You cannot use the bot. Contact developers to purchase a bot subscription.")
        return
    bot.reply_to(message, "Send the CC details in the format: `CC|MM|YY|CVV`")
    bot.register_next_step_handler(message, process_single_cc_check)
...
def process_single_cc_check(message):
    cc = message.text.strip()
    if not cc:
        bot.reply_to(message, "Please send the CC details in the format: `CC|MM|YY|CVV`")
        return
...
Bot By: <a href='t.me/BaignX'>BaignX</a>''')
            ...
            r2 = requests.post('https://api.stripe.com/v1/tokens', data={'card[number]': cc.split('|')[0], 'card[exp_month]': cc.split('|')[1], 'card[exp_year]': cc.split('|')[2], 'card[cvc]': cc.split('|')[3]}, auth=('sk_test_4eC39HqLyjWDarjtT1zdp7dc', ''))
            bot.reply_to(message, f"{cc.strip()} - DECLINED ❌ - {r2.json()['error']['message']}")
    except Exception as e:
        print(e)
        bot.reply_to(message, f"{cc} - Error processing card - Unknown")
